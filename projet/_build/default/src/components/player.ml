...
let player (name, x, y, txt, width, height) =
  ...
  e# resolve #set (fun _ t ->
    ...
    Wall.HWall (w) -> e#velocity#set Vector.zero;
                      if w#position#get.y==0. then e#position#set ( Vector.add (Rect.penetration_vector e#position#get w#box#get ) e#position#get)
                      else if w#position#get.y==568. then e#position#set ( Vector.sub (Rect.penetration_vector e#position#get w#box#get ) e#position#get )
    | _ -> ()
    );
